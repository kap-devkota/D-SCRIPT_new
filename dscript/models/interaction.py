...
class ModelInteraction(nn.Module):
    ...
    def __init__(
        ...
        self.glider_mat   = glider_mat
        self.glider_param = glider_param
        self.glider_map   = glider_map
        self.clip()

    def set_glider(glider_mat, glider_param, glider_map):
        self.glider_map = glider_map
        self.glider_mat = glider_mat
        self.glider_param = glider_param
        return

    def glider_score(prot1, prot2):
        for m in [prot1, prot2]:
            if m not in self.glider_map:
                return 0
            # might have to update this
        return self.glider_mat[self.glider_map[prot1], self.glider_map[prot2]]
