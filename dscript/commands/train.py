#!/cluster/tufts/cowenlab/.envs/dscript/bin/python
...
# glide functions
from dscript.models.glider import glide_compute_map #glide_compute_map(edges, thres = 0.5, params = {})
...
def interaction_grad(model, n0, n1, y, tensors, use_cuda, weight=0.35):
    ...
    if model.use_glider:
        ...
        for i in range(n_edges):
            g_score.append(model.glider_score(n0[i], n1[i]))
...
def main(args):
    ...
    output.flush()
    # Use train_df with score 1 to construct the network
    # Use this to compute glide_mat
    train_df = pd.read_csv(train_fi, sep="\t", header=None)
    glider_mat, glider_map  = glide_compute_map(train_df[train_df[2].isin([1.0, 1.0])].values.tolist())
    print("GLIDER computations done")
    if augment:
    ...
    if args.checkpoint is None:
        ...
        model = ModelInteraction(embedding, contact, use_W=use_W, pool_size=pool_width, glider_map = glider_map, glider_mat = glider_mat, glider_param = 0.5)
